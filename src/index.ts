...
const multisigMaster = Keypair.random();
...
(async () => {
  ...
  const router = new Contract(
    "CACIQ6HWPBEMPQYKRRAZSM6ZQORTBTS7DNXCRTI6NQYMUP2BHOXTBUVD" // Soroswap router (replace if needed)
  );
  ...
  const swapArgs: xdr.ScVal[] = [
    ...
    nativeToScVal(
      [
        ...
        "CA2D2WZ4OFT2XJLAY2IFSQFJJSNMIV4I4FQZOJ6DD6VQNIGOP7N24VZW",
      ].map((p) => new Address(p))
    ),
  ...
  const swapSim = await sendTx(swapTx, false, [signer1, signer2]);
  console.log(swapSim);
